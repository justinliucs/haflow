require([
	...
	"haflow/flow",
	"dojo/store/Memory",
	"dojo/data/ObjectStore","dojo/json","dojo/_base/lang"
	    ], function(require, array, config, dom, domClass, domConstruct, kernel, query, ready, win, fx, registry, MenuItem,
			locale, parser, ItemFileReadStore,ItemFileWriteStore, ForestStoreModel, number, source, json1, 
		    all, xContentPane, ContentPane, TabContainer, flow, Memory, ObjectStore, JSON, lang){
	 ...
	 flowTabOnClose = function(){
		 ...
		 if(close){
			 ...
			 for( var i = 0; i < item.erModule.length; i++ ){
				 ...
				 var confTabId = haflow.flow.getConfTabIdByModuleId(windowId);
...
//		     repaintModules(flowId);//paint flow modules
	...
	//invoked by menuClicked, paint flow modules on canvas only on the first time //openflow2
	repaintModules=function(flowId){	
		//clear the pane
		 ...
		 if(targetFlow && targetFlow.erModule.length > 0){//paint modules
	...
	runFlowInline = function(flowId){
		dojo.rawXhrPost({
           ...
           load: function(response) {
                ...
                domConstruct.place(domConstruct.toDom("<li>" + response.message + "</li>"), dom.byId("btab0console"));
	...
	fNewFlowNameCallBack = function(){
	 ...
	 newFlowInline = function(newFlowName){		
		...
		newFlowTab(newFlowName, getTabId(flowId), getCanvasId(flowId));//open a new flow tab
	 }
	 ...
	 newFlowTab = function(name, tabId, canvasId) {
	    var ntp = new dijit.layout.ContentPane({//a new tab when create a new flow
	        title: name,
	        style: "display:none; padding:10px;",
	        content: "<div  id=\""+ canvasId +"\" class=\"flowContent\" ><p></p></div>",
	        closable: true,
	        id: tabId,
	...
	rDeleteFlow = function(targetItem){
		currentTreeNode = dijit.byNode(targetItem.getParent().currentTarget);
		dojo.byId("flowDeleteConfirmContent").innerHTML = 
			"Are you sure to delete flow " + continentStore.getValue(currentTreeNode.item, "name");
		flowDeleteConfirmDialog.show();//make sure that user want to delete flow
	}
	
	//call back function of delete confirm dialog
	flowDeleteConfirmCallback = function(){
		flowDeleteConfirmDialog.hide();
		var tabid = dijit.byId(continentStore.getValue(currentTreeNode.item, "tabid"));
		if (tabid) {
		    dijit.byId("topTabs").removeChild(tabid);//close the tab of this tree node
		}
		continentStore.deleteItem(currentTreeNode.item);//remove the item from store
	}
	...
	runFlow = function() {
	    dojo.rawXhrPost({
	        ...
	        load: function(response) {
	            ...
	            domConstruct.place(domConstruct.toDom("<li>" + response.console + "</li>"), dom.byId("btab0console"));
	            dijit.byId("bottomTabs").selectChild("btab0");
	        }
	    });
	}
